...
# ---------------------------------------------------------------------------
# Справочники факультетов/групп — кэшируем в памяти процесса, чтобы не
# дёргать сайт заново на каждое нажатие кнопки
# ---------------------------------------------------------------------------
_faculties_cache: list[dict] | None = None
_groups_dict_cache: list[dict] | None = None


async def get_faculties_cached() -> list[dict]:
    global _faculties_cache
    if _faculties_cache is None:
        _faculties_cache = await ruz.get_faculties()
    return _faculties_cache


async def get_groups_dict_cached() -> list[dict]:
    global _groups_dict_cache
    if _groups_dict_cache is None:
        _groups_dict_cache = await ruz.get_groups_dictionary()
    return _groups_dict_cache


GROUP_SEARCH_CB = "group_search"
FACULTY_PICK_CB = "faculty_pick"
...
def group_picker_kb() -> InlineKeyboardMarkup:
    ...
    buttons.append([InlineKeyboardButton(text="🏫 Факультет → курс → группа", callback_data=FACULTY_PICK_CB)])
    buttons.append([InlineKeyboardButton(text="🔍 Другая группа (текстом)", callback_data=GROUP_SEARCH_CB)])
...
def faculties_kb(faculties: list[dict]) -> InlineKeyboardMarkup:
    buttons = []
    for f in faculties:
        oid = f.get("facultyOid")
        name = f.get("name") or f.get("abbr") or "?"
        if oid is None:
            continue
        buttons.append([InlineKeyboardButton(text=str(name), callback_data=f"fac:{oid}")])
    return InlineKeyboardMarkup(inline_keyboard=buttons)


def courses_kb(courses: list[int]) -> InlineKeyboardMarkup:
    buttons = [[InlineKeyboardButton(text=f"{c} курс", callback_data=f"crs:{c}")] for c in courses]
    return InlineKeyboardMarkup(inline_keyboard=buttons)


def dict_groups_kb(groups: list[dict]) -> InlineKeyboardMarkup:
    buttons = []
    for g in groups[:40]:
        oid = g.get("groupOid")
        label = g.get("name") or g.get("number") or "?"
        if oid is None:
            continue
        buttons.append([InlineKeyboardButton(text=str(label), callback_data=f"dgrp:{oid}")])
    return InlineKeyboardMarkup(inline_keyboard=buttons)
...
@dp.callback_query(F.data == FACULTY_PICK_CB)
async def cb_faculty_pick_start(callback: CallbackQuery):
    try:
        faculties = await get_faculties_cached()
    except RuzApiError as e:
        await callback.answer(f"Ошибка: {e}", show_alert=True)
        return
    if not faculties:
        await callback.answer("Список факультетов пуст.", show_alert=True)
        return
    await callback.message.answer("Выбери факультет:", reply_markup=faculties_kb(faculties))
    await callback.answer()


@dp.callback_query(F.data.startswith("fac:"))
async def cb_faculty_chosen(callback: CallbackQuery, state: FSMContext):
    faculty_oid = callback.data.split(":", 1)[1]
    try:
        groups = await get_groups_dict_cached()
    except RuzApiError as e:
        await callback.answer(f"Ошибка: {e}", show_alert=True)
        return
    matching = [g for g in groups if str(g.get("facultyOid")) == faculty_oid]
    if not matching:
        await callback.answer("У этого факультета не нашлось групп.", show_alert=True)
        return
    await state.update_data(faculty_oid=faculty_oid)
    courses = sorted({g.get("course") for g in matching if g.get("course") is not None})
    if len(courses) <= 1:
        await callback.message.answer(f"Групп: {len(matching)}. Выбери свою:", reply_markup=dict_groups_kb(matching))
    else:
        await callback.message.answer("Какой курс?", reply_markup=courses_kb(courses))
    await callback.answer()


@dp.callback_query(F.data.startswith("crs:"))
async def cb_course_chosen(callback: CallbackQuery, state: FSMContext):
    course = callback.data.split(":", 1)[1]
    data = await state.get_data()
    faculty_oid = data.get("faculty_oid")
    try:
        groups = await get_groups_dict_cached()
    except RuzApiError as e:
        await callback.answer(f"Ошибка: {e}", show_alert=True)
    ...
    matching = [
        g for g in groups
        if str(g.get("facultyOid")) == str(faculty_oid) and str(g.get("course")) == course
    ]
    if not matching:
        await callback.answer("Групп не нашлось.", show_alert=True)
        return
    await callback.message.answer(f"Групп: {len(matching)}. Выбери свою:", reply_markup=dict_groups_kb(matching))
    await callback.answer()


@dp.callback_query(F.data.startswith("dgrp:"))
async def cb_dict_group_chosen(callback: CallbackQuery, state: FSMContext):
